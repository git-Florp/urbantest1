...
export const WindowManager = ({ windows, onClose, onFocus, allWindows, onCloseWindow, onCriticalKill, onOpenAdminPanel, onLockdown }: WindowManagerProps) => {
  const getAppContent = (appId: string) => {
    switch (appId) {
      ...
      case "pdf-reader":
        return <GenericApp title="PDF Viewer" description="Read and annotate PDF documents" features={["View PDF files", "Add annotations", "Search text", "Print documents"]} />;
      case "spreadsheet":
      ...
      case "image-viewer":
        return <GenericApp title="Photo Gallery" description="View and organize images" features={["Thumbnail previews", "Slideshow mode", "Basic editing tools", "Batch operations"]} />;
      case "audio-editor":
